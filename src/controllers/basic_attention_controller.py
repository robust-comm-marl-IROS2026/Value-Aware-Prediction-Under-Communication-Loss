...
class BasicAttentionMAC:
    def __init__(self, scheme, groups, args):
        ...
        self.attention_network = MultiHeadAttention(d_model=self.agent_input_dim, comm_mask_dim=self.n_agents,
                                    d_head=ATT_HEAD_DIM, num_heads=NUM_ATT_HEADS)
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        att_inputs = att_inputs.view(bs,self.n_agents,self.agent_input_dim)
        comm_masks = agent_inputs[:,-self.masks_dim:] # [(bs*n_agents), n_agents]

        query_inputs = att_inputs.view(ep_batch.batch_size, self.n_agents, self.n_agents, self.agent_input_dim)

        parsed_query_inputs = []
        for a_id in range(self.n_agents):
            parsed_query_inputs.append(query_inputs[:,a_id,a_id,:]) # Get the local observation for each agent.
        query_inputs = th.stack(parsed_query_inputs, dim=1) # [(bs*n_agents),obs_dim]
        query_inputs = query_inputs.view(ep_batch.batch_size*self.n_agents, -1).unsqueeze(1) # [(bs*n_agents),1,obs_dim]

        att_context, attn = self.attention_network(query=query_inputs, key=att_inputs, value=att_inputs, comm_mask=comm_masks, mask=None) # [(bs*n_agents),1,att_out_dim]
        att_context = att_context.squeeze(1) # [(bs*n_agents),att_out_dim]

        agent_outs, self.hidden_states = self.agent(att_context, self.hidden_states)
    ...
    def save_models(self, path, logger=None, save_mongo=False):
        ...
        if logger is not None:
            logger.console_logger.info(f"Saved agent model to {path}/agent.th")
            if save_mongo:
                logger.log_model(filepath="{}/agent.th".format(path), name="agent.th")
                logger.console_logger.info(f"Uploaded agent model to MongoDB")
