...
class ParallelRunner:
    ...
    def reset(self, test_mode=False, comm_p_arg=None):
        ...
        if self.perc_model:
            perc_model_out = self.perc_model.encode(self.batch, t=0,
                        test_mode=test_mode, comm_p=comm_p_arg) # [batch_size_run,num_agents,latent_dim]
            pre_transition_data["obs"] = [[latent_a for latent_a in b]
                                                for b in perc_model_out.numpy()]
        self.action_selection_batch.update(pre_transition_data, ts=0)
    ...
    def run(self, test_mode=False, comm_p=None):
        ...
        self.reset(test_mode, comm_p_arg)
        ...
        while True:
            ...
            actions = self.mac.select_actions(self.action_selection_batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
            ...
            self.action_selection_batch.update(actions_chosen, bs=envs_not_terminated, ts=self.t, mark_filled=False)
            ...
            self.action_selection_batch.update(post_transition_data, bs=envs_not_terminated, ts=self.t, mark_filled=False)
            ...
            if self.perc_model:
                perc_model_out = self.perc_model.encode(self.batch, t=self.t,
                            test_mode=test_mode, comm_p=comm_p_arg) # [batch_size_run,num_agents,latent_dim]
                perc_model_out = [[latent_a for latent_a in b]
                                        for b in perc_model_out.numpy()]
                perc_model_out = [x for (i,x) in enumerate(perc_model_out)
                                    if i in envs_not_terminated] # Filter to get only not terminated envs.
                pre_transition_data["obs"] = perc_model_out
            self.action_selection_batch.update(pre_transition_data, bs=envs_not_terminated, ts=self.t, mark_filled=True)
        ...
        (len(self.test_returns) == n_test_runs):
